...
const minimumAbsDifference = (arr) => {
    ...
    for (let i = 1; i < arr.length; i++) {
        ...
        let minDiff = Math.abs(current[0] - current[1])
        if (minDiff < diff) {
            diff = minDiff;
            result = [];
        }
...
//             pairs.push(pair)
//             diff = Math.min(Math.abs(pair[0] - pair[1]), diff);
//         }

//     }
//     pairs = pairs.filter(e => Math.abs(e[0] - e[1]) === diff)
//     let truthy = false;
//     while (!truthy) {
//         truthy = true;
//         for (let i = 0; i < pairs.length; i++) {
//             if (pairs[i+1] !== undefined) {
//                 let forward = pairs[i+1];
//                 let current = pairs[i];
//                 if (current[0] + current[1] > forward[0] + forward[1]){
